...
function OrderForm() {
    ...
    const inputClass = (fieldName) => {
        const base = "w-full pl-12 pr-5 py-3.5 rounded-2xl border-2 bg-gray-50/50 focus:outline-none focus:ring-4 transition-all duration-300 font-medium text-gray-800 placeholder:text-gray-400";
        if (errors[fieldName]) {
    ...
    return (
        <div
            ...
            style={{
                minHeight: '100vh',
                width: '100%',
                background: 'linear-gradient(135deg, #e8f5e2 0%, #f5faf3 50%, #edf7e8 100%)',
                position: 'relative',
                display: 'flex',
                alignItems: 'center',
        ...
        >
            ...
            {/* Form Card */}
            <div className="bg-white/80 backdrop-blur-xl border border-white/80 shadow-[0_20px_60px_rgba(0,0,0,0.07)] w-full"
                style={{ maxWidth: '520px', borderRadius: '2.5rem', padding: 'clamp(2rem, 5vw, 2.5rem)', position: 'relative', zIndex: 10 }}
            >
                {isSuccess ? (
                    /* SUCCESS STATE */
                    <div className="text-center py-6 flex flex-col items-center gap-4">
                        ...
                        <p className="text-gray-500 text-sm max-w-xs">
                            Terima kasih, <strong className="text-[#84B179]">{formData.nama}</strong>! Pesanan sahurmu untuk kamar <strong className="text-[#84B179]">{formData.kamar}</strong> sedang diproses.
                        </p>
                        <div className="bg-[#84B179]/8 rounded-2xl p-4 w-full text-left space-y-2 border border-[#84B179]/20 mt-2">
                            ...
                            <div className="flex justify-between text-sm">
                                ...
                                <span className="font-bold text-gray-700">{formData.kamar}</span>
                            </div>
                            <div className="flex justify-between text-sm">
                                <span className="text-gray-500">Jumlah Porsi</span>
                                <span className="font-bold text-gray-700">{formData.jumlah} porsi</span>
                            </div>
                            {formData.keterangan && (
                                <div className="flex justify-between text-sm">
                                    <span className="text-gray-500">Keterangan</span>
                                    <span className="font-bold text-gray-700 max-w-[60%] text-right">{formData.keterangan}</span>
                                </div>
                            )}
                        </div>
                        <button
                    ...
                    <>
                        <div className="text-center mb-8">
                            ...
                            <h1 className="text-3xl font-extrabold text-gray-800 tracking-tight">Pesan Sahur</h1>
                            <p className="text-gray-400 mt-2 text-sm font-medium">Isi form di bawah biar puasanya makin kuat!</p>
                        </div>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-5" noValidate>
                            ...
                            <div className="space-y-1.5">
                                ...
                                <div className="relative">
                                    <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                                        <svg className={`w-5 h-5 ${errors.nama ? 'text-red-400' : 'text-gray-400'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                {errors.nama && <p className="text-xs text-red-500 font-medium pl-1 flex items-center gap-1">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                {/* Kamar */}
                                <div className="space-y-1.5">
                                    <label htmlFor="kamar" className="text-sm font-bold text-gray-700 block">
                                        No. Kamar <span className="text-[#84B179]">*</span>
                                    </label>
                                    <div className="relative">
                                        <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                                            <svg className={`w-5 h-5 ${errors.kamar ? 'text-red-400' : 'text-gray-400'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" />
                                            </svg>
                                        </div>
                                        <input
                                            type="text" id="kamar" name="kamar" required
                                    ...
                                    {errors.kamar && <p className="text-xs text-red-500 font-medium pl-1 flex items-center gap-1">
                                ...
                                {/* Jumlah */}
                                <div className="space-y-1.5">
                                    <label htmlFor="jumlah" className="text-sm font-bold text-gray-700 block">
                                        Jumlah Porsi <span className="text-[#84B179]">*</span>
                                    </label>
                                    <div className="relative">
                                        <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                                            <svg className={`w-5 h-5 ${errors.jumlah ? 'text-red-400' : 'text-gray-400'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
                                            </svg>
                                        </div>
                                        <input
                                            type="number" id="jumlah" name="jumlah" required min="1"
                                            placeholder="Berapa..."
                                    ...
                                    {errors.jumlah && <p className="text-xs text-red-500 font-medium pl-1 flex items-center gap-1">
                                        <svg className="w-3.5 h-3.5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clipRule="evenodd" /></svg>
                                        {errors.jumlah}
                                    </p>}
                                </div>
                            ...
                            <div className="space-y-1.5">
                                <div className="flex justify-between items-center">
                                    <label htmlFor="keterangan" className="text-sm font-bold text-gray-700">
                                        Keterangan <span className="text-gray-400 font-normal text-xs ml-1">(Opsional)</span>
                                    </label>
                                    <span className="text-xs font-medium text-gray-400">{formData.keterangan.length}/150</span>
                                </div>
                                <textarea
